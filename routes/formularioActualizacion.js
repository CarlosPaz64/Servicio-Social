...
module.exports = (io) => {
    ...
    const registrarActualizacion = async (req, res) => {
        ...
        try {
            ...
            const [newRow] = await pool.execute(
                'SELECT id, num_folio, id_unico, curp, fullname, tramite, comentario, turno FROM tramites_verificacion WHERE id = ?',
                [result.insertId]
            );
            ...
            console.error('Error al registrar la actualizacion de datos:', error);
            res.status(500).send('Ocurrió un error al registrar la actualizacion de datos.');
        }
    };
    ...
    router.post('/', registrarActualizacion);
